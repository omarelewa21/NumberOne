<head>
    ...
    <title>{{ $lesson->name }} | {{ $book->title }}</title>
...
<body>
        ...
        <nav class="nav container">
            ...
            <div class="main-header d-flex">
                <form class="inline-search-form header-search" action="/search/course">
                    @csrf
                    <input type="hidden" name="search_id" value="{{ $lesson->book->id }}">
            ...
            <div class="nav__menu" id="nav-menu">
                <ul class="nav__list">
                    ...
                    <li class="nav__item">
                            <a href="#" class="nav__link active-link" id="toggle">
                                <i class="bx bx-search-alt-2 nav__icon"></i>
                                <span class="nav__name">بحث</span>
                            </a>
                        </li>
                    @php
                        $previousresults = App\Models\QuizzResult::where('user_id', auth()->user()->id)
                    ...
                    @endphp
                        ...
                        @if (!$lesson->quiz->isEmpty())
                            <li class="nav__item">
                                <a href="/lesson/{{ $lesson->id }}/quiz/ " class="nav__link">
    ...
    <div class="slide-in from-left">
        <div class="slide-in-content">
            ...
            <form class="inline-search-form header-search " method="get" action="/search/course" style="display: block;">
                <input type="text" placeholder="بحث" name="keyword" autocomplete="off">
                <input type="hidden" name="search_id" value="{{ $lesson->book->id }}">
                <button type="submit">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
    ...
    <section class="course-overview-sections-wrap">
            ...
            <div class="c-menue">
                <ul class="navigation">
                    @foreach ($sections as $section)
                        <li class="nav-item"><a href="#{{ $section->slug }}"><span
                                    class="nav-link">{{ $section->title }}</span></a>
                        </li>
                    @endforeach
        ...
        <div class="contentContainer">
            @foreach ($sections as $section)
                <div class="texto" id= "{{ $section->slug }}">
                    <div class="bg-colo" style="background-color:#ffeb3b57; padding:10px; font-weight: 900;">
                        {!! $section->title !!}
                    </div>    
